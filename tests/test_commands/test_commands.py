...
async def test_good_log_create_with_flags(
	...
	target_part = " " + case["target"].getMessagePart() + " "
	din_part = case["d_in"].getMessagePart() + " "
	act_part = case["act"] + " "
	flags_part = case["flags"].getMessagePart()
	await dpytest.message("sudo log 1" + target_part + act_part +
						  din_part + flags_part)
	...
	async with db.cursor() as acur:
		...
		for row in await acur.fetchall():
			flags_values = list(map(lambda x: None if not x else x,
									case["flags"].values()
									))
