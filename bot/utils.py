...
class Case:
	...
	def __init__(self, **kwargs: Any):
		self.params_with_delayed_exprs: Dict[str, Any] = {}
	...
	def getDelayedExprs(self) -> Dict[str, Any]:
		return self.params_with_delayed_exprs
...
class DelayedExpressionEvaluator:
	...
	def __init__(self,
					 params_for_eval: Dict[param := str, Case],
		...
		Args:
			...
			an expression eval.
		"""
		self.params_for_eval = params_for_eval
		self.global_vars = global_vars

	def eval(self) -> Dict[str, Case]: #! якорь
		for param, case_instance in self.params_for_eval:
			if case_instance.hasDelayedExprs:
				delayedExprs = case_instance.getDelayedExprs()

	@staticmethod
	def preparePytestFuncAttrsToDelayExprEval(
			params_from_func: Dict[str, Case],
			params_and_fixtures: Dict[str, Any]) -> Tuple[
		Dict[str, Case], Dict[str, Any]]:
		...
		return params_with_case, fixtures
...
class DelayedExpressionsStorage(list):

	def __init__(self, delayed_expr: List[DelayedExpression]):
		super().__init__(delayed_expr)

	def eval(self, global_vars: Dict[str, Any]):
		for ind in len(self):
			self.__getitem__[ind].eval(global_vars)
